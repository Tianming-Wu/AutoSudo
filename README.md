...
此项目刚经过一次主要的技术架构更新，在确认稳定之前不会发布正式版本。你可以尝试使用测试版，它们不能跟稳定版本共存，也不与之前的配置文件兼容（但是保留的话不会发生冲突，因为使用不同的配置文件存储方式）。
...
AutoSudo 2.0 (Gen 4) 使用强大的规则引擎替代了旧的允许列表机制。规则引擎支持多种匹配条件：
...
- 规则引擎中的正则表达式规则虽然强大，但是不恰当的正则表达式可能导致性能问题（都知道Cloudflare的那次事故罢）。
- AutoSudoGUI 还在开发中，目前仅有基础功能，并未支持所有特性。
- 路径匹配规则目前只能大小写敏感。目前的引擎架构并不支持额外添加大小写不敏感的选项，建议养成使用保持一致大小写的好习惯。如果目前急需，可以考虑使用更简单的匹配规则，或者使用正则表达式临时替代。

## 开发状态

🚧 **规则引擎实现阶段** - 核心规则引擎完成，正在完善 GUI 管理工具和高级特性。

目前，AutoSudo 已经可以用于日常使用场景。
...
- [x] **规则引擎系统** - 替代旧的允许列表
- [x] 用户交互确认
- [x] **AutoSudoSdk 库** - 供外部 API 级别调用
- [x] 规则 CRUD 操作
- [x] 规则排序和管理
- [x] 多种规则类型（路径、哈希、签名等）
- [x] 自动审计规则
- [x] 支持对信任的数字签名可执行文件自动审计
...
- [ ] 加密管道通信（等待上游 SharedCppLib2 完成加密接口）
- [ ] 添加参数安全控制规则
- [ ] 加密配置文件（等待上游接口）
- [ ] 加密凭据使用系统密钥存储（注：最严重的后果只是丢失允许列表）
- [ ] 完善日志和审计功能（这估计永远不可能完全做完，总是有新东西要改）
- [ ] 允许自定义 session ID（支持在其他激活用户会话中运行）
- [ ] 防止重放攻击
- [ ] Lua 脚本规则（暂时无法引入 lua 引擎，可能作为一个插件加载）
- [ ] REST API 接口
